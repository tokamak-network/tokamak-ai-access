...
- 총 스테이킹 ≥ 10 TON이면 "✅ Eligible" 표시
- 총 스테이킹 < 10 TON이면 "❌ Not Eligible" + 스테이킹 안내 링크
...
- 새 키는 1회만 화면에 표시된다
- Rotate 중 자격 재검증이 이루어진다 (< 10 TON이면 403)
...
**Acceptance Criteria:**
- 현재 스테이킹 잔액과 필요 잔액(10 TON)이 명시된다
...
3. **스테이킹 임계값** 10 TON은 `MIN_TON` 환경변수로 변경 가능 (PoC 기본값).
