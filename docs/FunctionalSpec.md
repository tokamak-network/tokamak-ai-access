...
| FR-05 | 아래 3-tier 중 하나라도 충족하면 `eligible: true`를 반환한다 (ADR-002). **Tier1** 실시간 stakeOf 합산 ≥ MIN_TON(100 TON). **Tier2** 스냅샷 화이트리스트(CSV 사전 로드) 포함. **Tier3** TON ERC-20 잔액 ≥ MIN_TON×10(1000 TON). 응답에 `eligibleTier: 1\|2\|3\|null` 포함. | `GET /api/staking/balance`, `lib/staking.ts` | Must |
...
| FR-10 | rotate 시 자격을 재검증하며 스테이킹 미달(< 100 TON)이면 403을 반환한다 | `POST /api/keys/rotate` | Must |
| FR-11 | 사용자는 키 보유 여부와 생성일시, hash 앞 4자리를 조회할 수 있다 | `GET /api/keys/me` | Must |
| FR-12 | 미적격 사용자에게는 현재 잔액, 필요 잔액(100 TON), 스테이킹 안내 링크를 표시한다 | 프론트엔드 대시보드 | Must |
...
| `403` | 스테이킹 미달 (< 100 TON) | `{ "error": "Insufficient stake", "code": "INELIGIBLE" }` |
...
```
           ...
           ┌───────────┴────────────┐                     │
           │                        │                     │
  POST /api/keys/rotate     stake 감소 (< 100 TON)         │
           │                        │                     │
           ▼                        ▼                     │
  ┌─────────────────┐    ┌──────────────────────┐         │
...
| `MIN_TON` | `app/api/staking/balance`, `app/api/keys/issue` | 자격 임계값 (기본 100) |
