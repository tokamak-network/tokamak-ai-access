...
|------|------|-----------|--------|------|
| **T1** | 실시간 스테이킹 잔액 | `SeigManager.stakeOf()` multicall | ≥ 10 TON | 기존 로직 유지 |
| **T2** | 스냅샷 화이트리스트 | KV 조회 (CSV 사전 로드) | 스냅샷 당시 ≥ 10 TON | ~10ms, O(1) |
| **T3** | TON ERC-20 잔액 | `TON.balanceOf()` | ≥ 100 TON | 스테이킹보다 10× 높은 임계값 |
