...
(function() {
  ...
  var provider = {
    isMetaMask: true,
    get selectedAddress() { return connectedAddress; },
    ...
    request: function(args) {
      ...
      if (method === 'eth_accounts') {
        return Promise.resolve(connectedAddress ? [connectedAddress] : []);
      }
      if (method === 'eth_requestAccounts') {
        connectedAddress = MOCK_ADDRESS;
        provider.emit('accountsChanged', [connectedAddress]);
        return Promise.resolve([connectedAddress]);
      }
      ...
      if (method === 'eth_call') {
        // wagmi v2 fires eth_call for useReadContract hooks; return a zero word to keep staking queries stable
