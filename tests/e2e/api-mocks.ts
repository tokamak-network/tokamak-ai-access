...
export async function applyAuthMocks(page: Page): Promise<void> {
  ...
  await page.route('**/api/auth/verify', (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      // Set a session cookie so the dashboard fetch loop doesn't 401
  ...
  await page.route('**/api/keys/me', (route) =>
    route.fulfill({
      ...
      body: JSON.stringify({
        hasActiveKey: true,
        lastFour: '7890',
        createdAt: isoAgo(THIRTY_ONE_DAYS_MS), // 31 days ago → renewable
...
export async function applyIneligibleMocks(page: Page): Promise<void> {
  ...
  await page.route('**/api/price/ton', (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({ tonRequired: 42, usdPrice: 5, usdPerTon: 0.12 }),
    }),
  );
